...
module.exports = React.createClass({
	...
	componentDidMount: function() {
		LandmarksStore.on('change', () => {
			this.setState({data: LandmarksStore.data});
		});
	},*/
	render: function() {
		//var landmarkDots = {};
		...
		var estimatedPosition = this.state.data.estimatedPosition;//{x: 10, y: 18};
		var landmarkRange = this.state.data.landmarkRange;//getLandmarkRange(estimatedPosition);
		//var readings = simulateLidar(9, 18, 2000);
		var landmarks = this.state.data.allLandmarks;//findLandmarks(readings);
