...
/* Debian rev c: */
var start = Date.now();
shell.exec('rsync -aWu -e "ssh -o StrictHostKeyChecking=no -o' +
	'UserKnownHostsFile=/dev/null -c arcfour" --progress --recursive --exclude=.git* ' +
...
/*shell.exec('rsync -aWuv -e "sshpass -p ' + secret.password + ' ssh -o StrictHostKeyChecking=no -o' +
	'UserKnownHostsFile=/dev/null -c arcfour" --progress --recursive --exclude=.git* ' +
	'--exclude=node_modules ./../robot/ ubuntu@' + secret.sshHost + ':' + secret.sshPath, function() {
		console.log('Uploaded in ' + (Date.now() - start)/1000 + ' seconds');
	});*/
